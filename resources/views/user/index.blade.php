<x-app-layout>
                ...
                <div class="lg:flex items-center">
                    <select name="category" class="mb-2 lg:mb-0 lg:mr-2" >
                       <option value="0" @if(\Request::get('category') === '0') selected  @endif>全て</option>
                        @foreach ($categories as $category){{-- メインカテゴリを回す --}}
                            <optgroup label="{{ $category->name }}"> {{-- メインカテゴリの名前を表示 --}}
                                @foreach ($category->secondary as $secondary) {{--メインカテゴリに紐づくセカンダリカテゴリを全件取得--}}
                ...
                <div>
                    ...
                    <select name="sort" id="sort" class="mr-4">
                        ...
                        <option value="{{ \Constant::SORT_ORDER['higherPrice'] }}"
                            @if (\Request::get('sort') === \Constant::SORT_ORDER['higherPrice']) selected @endif> 価格の高い順
                        </option>

                        <option value="{{ \Constant::SORT_ORDER['lowerPrice'] }}"
                            @if (\Request::get('sort') === \Constant::SORT_ORDER['lowerPrice']) selected @endif> 価格の低い順
                        </option>

                        <option value="{{ \Constant::SORT_ORDER['later'] }}"
                            @if (\Request::get('sort') === \Constant::SORT_ORDER['later']) selected @endif> 出品日が新しい順
                        </option>

                        <option value="{{ \Constant::SORT_ORDER['older'] }}"
                            @if (\Request::get('sort') === \Constant::SORT_ORDER['older']) selected @endif> 出品日が古い順
                        </option>
                    </select>
                </div>
    ...
    <script>
        ...
        paginate.addEventListener('paginaiton', function() {
            this.form.submit()
        })
    </script>
</x-app-layout>
